...
public class ConsoleManageMovieView {
    ...
    private final IManageMovieController movieController;
    
    private boolean running;
    
    public ConsoleManageMovieView(IManageMovieController movieController) {
    ...
    private Set<Language> getLanguages(Language[] languages) {
        ...
        for(String choice : languagesChoice.split(" ")) {
            try {
                ...
                languageList.add(languages[index - 1]);
            } catch(NumberFormatException e) {
                displayError("Invalid input");
            }
        }
